...
namespace Troop
{
    public partial class TroopManager : ManualSingletonMono<TroopManager>
    {
        ...
        [Tooltip("Có sử dụng spatial partitioning không")]
        ...
        public void RegisterTroop(TroopController troop)
        {
            ...
            if (useSpatialPartitioning && Core.Optimization.TroopSpatialPartitioning.Instance)
            {
                Core.Optimization.TroopSpatialPartitioning.Instance.RegisterTroop(troop);
            }
        }
        ...
        public void UnregisterTroop(TroopController troop)
        {
            ...
            if (useSpatialPartitioning && Core.Optimization.TroopSpatialPartitioning.Instance)
            {
                Core.Optimization.TroopSpatialPartitioning.Instance.UnregisterTroop(troop);
            }
        }
        ...
        public TroopController[] GetNearbyTroopsOptimized(TroopController troop)
        {
            if (!useSpatialPartitioning || Core.Optimization.TroopSpatialPartitioning.Instance == null)
            {
                return GetNearbyAllies(troop);
            }
            List<TroopController> allies = Core.Optimization.TroopSpatialPartitioning.Instance.GetAlliesInRadius(
                troop.GetPosition(), 
                nearbyTroopDetectionRadius,
                troop.gameObject.tag);
                
            return allies.ToArray();
        }public TroopController[] GetNearbyEnemiesOptimized(TroopController troop)
        {
            if (!useSpatialPartitioning || Core.Optimization.TroopSpatialPartitioning.Instance == null)
            {
                return GetNearbyEnemies(troop);
            }
            List<TroopController> enemies = Core.Optimization.TroopSpatialPartitioning.Instance.GetEnemiesInRadius(
                troop.GetPosition(), 
                nearbyTroopDetectionRadius,
                troop.gameObject.tag);
