...
namespace Generate
{
    public class IslandGenerator : MonoBehaviour
    {
        ...
        // Thêm vào class IslandGenerator
        public void AddCustomValidPosition(Vector3 worldPosition)
        {
            // Chuyển từ world position sang grid position
            int gridX = Mathf.RoundToInt((worldPosition.x + (islandWidth * cellSize / 2)) / cellSize);
            int gridZ = Mathf.RoundToInt((worldPosition.z + (islandLength * cellSize / 2)) / cellSize);
            ...
            if (gridX >= 0 && gridX < islandWidth && gridZ >= 0 && gridZ < islandLength)
            {
                _validPositions[gridX, gridZ] = true;
        
                Vector3 actualPos = new Vector3(
                    gridX * cellSize - (islandWidth * cellSize / 2),
                    0,
                    gridZ * cellSize - (islandLength * cellSize / 2)
                );
        
                _validWorldPositions.Add(actualPos);
        
                // Tạo visual marker
                if (validPositionPrefab != null)
                {
                    Vector3 worldPos = GetWorldPositionOnIsland(gridX, gridZ);
                    GameObject marker = Instantiate(validPositionPrefab, worldPos, Quaternion.identity, transform);
                    marker.name = "Marker_Custom_" + gridX + "_" + gridZ;
                    _positionMarkers.Add(marker);
                }
            }
        }
        public void AddCustomTile(Vector3 position)
        {
            int gridX = Mathf.RoundToInt((position.x + (islandWidth * cellSize / 2)) / cellSize);
            int gridZ = Mathf.RoundToInt((position.z + (islandLength * cellSize / 2)) / cellSize);
            ...
            if (gridX >= 0 && gridX < islandWidth && gridZ >= 0 && gridZ < islandLength)
            {
                ...
                {
                    GameObject marker = Instantiate(validPositionPrefab, worldPos, Quaternion.identity, transform);
                    marker.name = "CustomMarker_" + gridX + "_" + gridZ;
                    _positionMarkers.Add(marker);
                }
        
                Debug.Log("Added custom tile at " + worldPos);
            }
        }
    }
}